...
async function compileLess(lessContent: string, savePath: string, rootPath: string, min?: boolean) {
  ...
  if (min) {
    lessOptions.plugins = [new LessPluginCleanCSS({ advanced: true })]
  }
...
export const componentsBuildStyle: TaskFunction = async done => {
  ...
  const componentDirs = paths.filter(
    path => isDirectory(join(componentsRootDir, path)) && !unComponentDir.includes(path),
  )
